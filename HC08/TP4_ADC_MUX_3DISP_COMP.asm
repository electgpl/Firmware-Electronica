...
INICIO 	clrx					;Inicializa acumulador X	
		...
		beq		SIGUE1
		bra		SIGUE2
SIGUE1	mov		#$FF,varMue
		jsr		LEEADC			;Rutina de lectura de ADC
SIGUE2	jsr		DECTBCD			;Rutina de conversion de Decimal a BCD	
...
LEEADC	brclr	7,ADSCR,LEEADC	;Espera a que bit COCO sea 1
		...
		jsr		DELA25			;Llama a rutina de Delay 2,5ms
		ldx		#$05			;Valor 5
		div						;A/X=A
		ldx		#$02			;Valor 2
		mul						;A*X=A		
...
;- (9+(10*249))+5=2503us=+/-2,5ms                                        -
;-------------------------------------------------------------------------
DELA25	mov		#$F9,varDel		;[5]Se carga el valor 249 a varDel
